...
def a_k(k, T, func, points):
    result, err = quad(lambda t: func(t, points) * np.cos(2 * np.pi * k * t / T), a, b)
    return 2 / T * result, err  # Возвращаем погрешность интегрирования

def b_k(k, T, func, points):
    result, err = quad(lambda t: func(t, points) * np.sin(2 * np.pi * k * t / T), a, b)
    return 2 / T * result, err  # Возвращаем погрешность интегрирования

# Вычисление амплитуды и фазы с погрешностью
def calculate_amplitudes_and_phases(N, T, func, points):
    ...
    for k in range(1, N + 1):
        a, a_error = a_k(k, T, func, points)
        b, b_error = b_k(k, T, func, points)
        
        amplitude = np.sqrt(a**2 + b**2)
        amplitude_error = np.sqrt((a * a_error)**2 + (b * b_error)**2)  # Погрешность амплитуды

        # Погрешность фазы с учетом производной функции арктангенс
        if amplitude != 0:  # Чтобы избежать деления на ноль
            da = a_error / amplitude
            db = b_error / amplitude
            phase_error = np.sqrt(((-b / (a**2 + b**2)) * da)**2 + ((a / (a**2 + b**2)) * db)**2)
            phase_error = phase_error * (180 / np.pi)  # Перевод в градусы
        else:
            phase_error = 0
    ...
    return amplitudes, phases, amplitude_errors, phase_errors
...
# Печать гармоник с погрешностями
def print_harmonics(a0, amplitudes, phases, amplitude_errors, phase_errors, real_phases, label):
    print(f"Гармоники для {label}:")
    print(f"  Постоянная компонента: A_0 = {a0:.3f}")
    for k, (amplitude, amplitude_error, phase, phase_error, real_phase) in enumerate(zip(amplitudes, amplitude_errors, phases, phase_errors, real_phases), start=1):
        print(f"  Гармоника {k}: A = {amplitude:.3f} ± {amplitude_error:.3f}, φ_theor = {phase:.0f}° ± {phase_error:.0f}°, φ_real = {real_phase}°")
...
# Подготовка данных
a0_x = a_k(0, T, piecewise_linear_signal, points_x)[0] / 2
amplitudes_x, phases_x, amplitude_errors_x, phase_errors_x = calculate_amplitudes_and_phases(N, T, piecewise_linear_signal, points_x)
real_phases_x = get_real_phases(phases_x)

a0_y = a_k(0, T, piecewise_linear_signal, points_y)[0] / 2
amplitudes_y, phases_y, amplitude_errors_y, phase_errors_y = calculate_amplitudes_and_phases(N, T, piecewise_linear_signal, points_y)
real_phases_y = get_real_phases(phases_y)

# Печать гармоник
if print_amplitudes_x:
    print_harmonics(a0_x, amplitudes_x, phases_x, amplitude_errors_x, phase_errors_x, real_phases_x, "x(t)")
if print_amplitudes_y:
    print_harmonics(a0_y, amplitudes_y, phases_y, amplitude_errors_y, phase_errors_y, real_phases_y, "y(t)")

# Построение графиков x(t), y(t)
t_vals = np.linspace(a - T/2, T, 1000)
